...
寻找单路数据的峰值
"""
def find_peek(data,amplitude = 0.6):
    ...
    for i in range(10, len(data) - 10):
        ...
        if data[i] > data[i + 5] and data[i] > data[i - 5] and data[i] > threshold:
            if len(peek_list) > 1:
                diff = i - peek_list[-1]
            else:
                diff = 101
...
寻找一组数据的峰值和谷值
"""
def peekAndVally(data):

    mean = find_mean(data)
    peek_idx = find_peek(data)
    vally_data = [mean + mean-(_) for _ in data]
    vally_idx = find_peek(data)
    return peek_idx, vally_idx
